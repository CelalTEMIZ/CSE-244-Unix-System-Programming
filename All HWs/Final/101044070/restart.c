...
ssize_t message_write(int fd, void *buf, size_t size, int id)
{
	...
	message.id = id;
	message.size = size;
	memcpy(message.message, bufp, size);
	
	return r_write(fd, &message, sizeof(messagePacket));
}

ssize_t message_read(int fd, void *buf, int id)
{
	messagePacket message;
	char* bufp = buf;

	r_read(fd, &message, sizeof(messagePacket));
	
	if (message.id != id)
	{
		r_write(fd, &message, sizeof(messagePacket));
		return -2;
	}
	
	memcpy(bufp, message.message, message.size);
	bufp[message.size] = 0;
	
	return message.size;
}

ssize_t message_readtimed(int fd, void *buf, size_t nbyte, double seconds, int id)
{
	struct timeval timedone;
